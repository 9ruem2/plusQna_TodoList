...
public class BoardManager {
    ...
    public QnaForum boardUpdater(QnaForum qnaForum) {
        ...
        QnaForum findQnaForum = findVerifiedBoard(qnaForum.getBoardId()); // FindBoard은 원본게시글이 저장됨

        //게시글을 등록한 멤버Id와 업데이트를 요청 멤버의 id가 같은지 확인하기
        checkNotExistBoard(qnaForum);
    ...
    public void checkNotExistBoard(QnaForum qnaForum){// 업데이트할 내용이 저장되어있는 board라고 가정
        ...
        for(QnaForum b: qnaForumList){
            if(qnaForum.getBoardId() == b.getBoardId()){ //업데이트를 요청한 멤버의 게시글목록중 업데이트요청 게시글의 boardId가 있는지 확인
    ...
    public QnaForum changeContent(QnaForum qnaForum){
        QnaForum findQnaForum = findVerifiedBoard(qnaForum.getBoardId());
